package org.borrowed.ldcbenc.api;

/**
 * * TODO add dependency when available in mvn repo, borrowed from
 * {@link <a href=
 * "https://github.com/dice-group/orca/blob/develop/ldcbench.api/src/main/java/org/dice_research/ldcbench/graph/GraphBuilder.java">org.dice_research.ldcbench.graph.GraphBuilder</a>}
 * 
...
public interface GraphBuilder extends Graph {
	 ...
	 * @param typeId   the type id of the edge
	 ...
	 * {@link #INTERNAL_NODE_GRAPH_ID} as graph ID meaning that they are part of
	 * this graph. However, to mark external nodes (i.e., nodes that are not part of
	 * this graph but are linked from one of the nodes of this graph), this method
	 * can be used to store their graph ID as well as their ID in the other graph.
	 * 
	 * @param nodeId     the ID of the node in this graph
	 * @param graphId    the ID of the graph in the other graph. Use
	 *                   {@link #INTERNAL_NODE_GRAPH_ID} to delete the marking of a
	 *                   node being external.
	 * @param externalId the ID of this node inside the other graph (only used if
	 *                   the graphID != {@link #INTERNAL_NODE_GRAPH_ID})
	 */
