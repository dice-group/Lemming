...
//				.getmTriColosCountsAvgProb();
		...
		try (ProgressBar vertsPB = new ProgressBarBuilder().setTaskName("#vertices").setInitialMax(desiredVertices)
			...
			if ((initialProposer != null) && (estimatedEdgesTriangle >= 3) && (estimatedVerticesTriangle >= 3)) {
				...
				double[] arrTriProbCount = mTriColosCountsAvgProb.get(initialTriangle.getA())
						.get(initialTriangle.getB()).get(initialTriangle.getC());
					...
					if ((mimicGraph.getNumberOfVertices() < estimatedVerticesTriangle)
						...
						if (edgeColosSet.size() != 0) {
							...
							if (proposedVertex3Colo != null) {
								...
								double[] arrNewPossTriProbCount = new double[5];
								var mapA = mTriColosCountsAvgProb.get(newPossibleTriangle.getA());
								if (mapA != null) {
								    var mapB = mapA.get(newPossibleTriangle.getB());
								    if (mapB != null) {
								        var value = mapB.get(newPossibleTriangle.getC());
								        if (value != null) {
								            arrNewPossTriProbCount = value;
								        } 
								    } 
								}
								...
								while (arrNewPossTriProbCount[3]+tolerance < 1 && iterations < 100) {
									...
									if (proposedVertex3Colo != null) {
										newPossibleTriangle = new TriColours(selectedVertex1Colo, selectedVertex2Colo,
												proposedVertex3Colo);
										arrNewPossTriProbCount = new double[5];
										mapA = mTriColosCountsAvgProb.get(newPossibleTriangle.getA());
										if (mapA != null) {
										    var mapB = mapA.get(newPossibleTriangle.getB());
										    if (mapB != null) {
										        var value = mapB.get(newPossibleTriangle.getC());
								...
								if (arrNewPossTriProbCount[3]+tolerance >= 1) {
									...
									Integer selectedVertex1 = getProposedVertex(
											initializer.getmMapColourToVertexIDs2Simplex(), selectedVertex1Colo);
									Integer selectedVertex2 = getProposedVertex(
											initializer.getmMapColourToVertexIDs2Simplex(), selectedVertex2Colo);
									if (selectedVertex1 != null && selectedVertex2 != null) {
										...
										simplexProperty.addEdgeToMimicGraph(mimicGraph, selectedVertex1Colo,
												proposedVertex3Colo, selectedVertex1, proposedVertId,
												initializer.getmMapColourToEdgeIDs2Simplex(),
												analysis.getConnTriAnalysis().getmColourMapperSimplexes(),
												newPossibleTriangle, simplexProperty.getmPropDistConnTri());
										simplexProperty.addEdgeToMimicGraph(mimicGraph, selectedVertex2Colo,
												proposedVertex3Colo, selectedVertex2, proposedVertId,
												initializer.getmMapColourToEdgeIDs2Simplex(),
												analysis.getConnTriAnalysis().getmColourMapperSimplexes(),
												newPossibleTriangle, simplexProperty.getmPropDistConnTri());

										// increment number of vertices and edges added in the mimic graph for triangles
										actualVerticesInTriangles++;
										...
										edgeColosSet
												.add(new EdgeColorsSorted(selectedVertex1Colo, proposedVertex3Colo));
										edgeColosSet
												.add(new EdgeColorsSorted(selectedVertex2Colo, proposedVertex3Colo));

										// Track the triangle colors along with vertex ids
										updateMapTriangleColorsVertices(selectedVertex1, selectedVertex2,
												proposedVertId, newPossibleTriangle,
												initializer.getmTriangleColorsVertexIds());
							...
							if (newEdgesNotAddedToTriangle) {
								numOfIterationAddingEdgesToGraph++;
								if (numOfIterationAddingEdgesToGraph == maximumIteration) { 
							...
							double[] arrNewTriProbCount = new double[5];
							var mapA = mTriColosCountsAvgProb.get(randomTriangle.getA());
							if (mapA != null) {
							    var mapB = mapA.get(randomTriangle.getB());
							    if (mapB != null) {
							        var value = mapB.get(randomTriangle.getC());
							        if (value != null) {
							        	arrNewTriProbCount = value;
							        }
							    } 
							} // get count of triangle for
							...
							if (arrNewTriProbCount[3]+tolerance >= 1) {
								...
								setTriangleColorsMimicGraph.add(new TriColours(randomTriangle.getA(),
										randomTriangle.getB(), randomTriangle.getC()));
								numOfIterationAddingEdgesToGraph = 0;
							} else {
								break;
							}
						...
						for (List<Integer> pairOfVertices : differentPairsOfVertices) {
							...
							if (edgeAdded) {
								...
								numOfIterationAddingEdgesToGraph = 0;// update count to 0 since edge was added
																		// successfully
								LOGGER.debug("Current edges: "+actualEdgesInTriangles+" out of estimated "+estimatedEdgesTriangle);
...
//				.getPotentialIsolatedEdgeColoProposer();
			...
			while ((estimatedEdges1Simplexes > actualEdgesSimplexes) && (potentialEdgeColoProposer != null)
				...
				boolean edgeAdded = simplexProperty.addEdgeWithTriangleCheck(mimicGraph, potentialheadColo,
						potentialTailColo, vertexIDHead, vertexIDTail, initializer.getmMapColourToEdgeIDs1Simplex(),
			...
			while ((estimatedEdgesIsoSelfLoop > actualEdgesSimplexes) && (potentialHeadColoProposer != null)
				...
				boolean edgeAdded = simplexProperty.addEdgeWithTriangleCheck(mimicGraph, potentialColoSelfLoop,
						potentialColoSelfLoop, vertexIDHead, vertexIDTail, initializer.getmMapColourToEdgeIDs1Simplex(),
				...
				while ((actualEdgesSimplexes < estimatedEdges1SimplexesConnect2Simplexes)
					...
					if (potentialHeadColoIsolatedTri != null) { // check for tail color only if head color is not null
						...
						if (potentialTailColoIsolatedTri != null) { // try to connect triangles using 1-simplexes if
							...
							if(possVerticesListhead.isEmpty()) {
								numOfIterationAddingEdgesToGraph++;
								continue;
							}
							...
							if(possVerticesListtail.isEmpty()) {
								numOfIterationAddingEdgesToGraph++;
								continue;
							}
				...
				if (potentialItem != null) {
					...
					if ((potentialHeadColocase4b != null) && (potentialTailColocase4b != null)) {
						...
						while ((numIterationsCase4b < 51)) {
							...
							boolean edgeAdded = simplexProperty.addEdgeWithTriangleCheck(mimicGraph,
									potentialHeadColocase4b, potentialTailColocase4b, possHeadIDcase4b,
									possTailIDcase4b, initializer.getmMapColourToEdgeIDs1Simplex(),
...
//								LOGGER.debug("Successfully connected triangles using 1-simplex (Case 4b)");
				...
				while ((actualEdgesInConnS1 < estEdgesConnected1Simplexes)
					...
					if (setEdgeColorsMimicGraph.size() != 0) {
						...
						if (edgeAdded) {
							...
							edgeAdded = simplexProperty.addEdgeWithTriangleCheck(mimicGraph, existingVertexColo2,
									existingVertexColo1, existingVertexID2, existingVertexID1,
									initializer.getmMapColourToEdgeIDs1Simplex(),
									analysis.getConnS1Analysis().getmColourMapperSimplexes(), false,
			...
			while ((actualEdgesSimplexes < estimatedEdgesCommon) && (potentialHeadColoCommon2Simplex != null)
				...
				boolean havingVertices = initializer.commonVertices(mimicGraph, vertexIDWithHeadColo,
						vertexIDWithTailColo);
				...
				if ((possibleLinkingEdgeColours.size() > 0) && !havingVertices) {
					...
					initializer.updateMappingOfEdgeColoHeadTailColo(possEdgeColo, vertexIDWithHeadColo,
							vertexIDWithTailColo);
	...
	public Integer getProposedVertex(Map<BitSet, IntSet> map, BitSet vertexColour) {
		Integer result = null;
		if(map.get(vertexColour) == null) {
			return result;
		}
