...
/**
 * This class models an algorithm for computing the amount of node triangles in a given (large)
 * graph. This is done using the Duolion algorithm proposed by Tsourakakis et al. (see below) which
 * applies a sparsification process onto the original graph, such that each edge is removed with a
 * certain probability. Once this is done, a standard node triangle counting algorithm is used to
 * compute the amount of triangles on the resulting graph. Finally the amount of triangles computed
 * on the smaller graph is multiplied by a factor based on the edge removal probability.
 *
 * @see <a href=
