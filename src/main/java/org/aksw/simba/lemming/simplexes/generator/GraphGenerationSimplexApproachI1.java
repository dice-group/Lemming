...
/**
 * Description of different cases for simplexes
 * Connected 2-simplexes: Random Triangle, Approach for adding new triangle: Edge selection random, and vertex is added based on probability. Adding edges to triangle: Probability based.
 * Isolated 2-simplexes: Selected based on their probability.
 * Connected 1-Simplexes: Random Edge, Approach for adding new edge to it: Select a head node based on probability distribution and tail node at random. Adding more edge : Probability based.
 * Isolated 1-simplexes: Selected based on their probability.
 * 0-simplexes: Selected based on their probability.
 * 1-Simplexes connecting 2-simplexes: Selected based on their probability
 * 1-Simplexes connected to 2-simplexes: Selected based on their probability.
 * 
 */
public class GraphGenerationSimplexApproachI1 extends AbstractGraphGeneration implements IGraphGeneration {
	...
	private ObjectObjectOpenHashMap<BitSet, IntSet> mMapColourToVertexIDs1SimplexConnected = new ObjectObjectOpenHashMap<BitSet, IntSet>(); // Map for storing vertex colours as Keys and Vertex IDs as Value for connected 1-simplexes
	...
	public GraphGenerationSimplexApproachI1(int iNumberOfVertices,
	...
	public ColouredGraph generateGraph(){
		...
		int estEdgesConn1Simplexes = estimatedEdgesConnS1;
				
		int actualVerticesMimicGraph = mMimicGraph.getVertices().size();
		int estVerticesConn1Simplexes = mIDesiredNoOfVertices - actualVerticesMimicGraph - estVerts1SimplexesConntoTri; // subtracting estimated number of vertices for 1-simplexes connected only to triangles. Such vertices are created in next step.
		//int verticesConnected1Simplexes = estimatedVertsConnS1;
		
		LOGGER.info("Case 6: Connected 1-simplexes");
		LOGGER.info("Estimated Edges: " + estEdgesConn1Simplexes);
		LOGGER.info("Estimated Vertices: " + estVerticesConn1Simplexes);
		...
		if ((initialRandomEdge!=null) && (estEdgesConn1Simplexes >= 1) && (estVerticesConn1Simplexes >=2)) { 
			...
			estVerticesConn1Simplexes = estVerticesConn1Simplexes - numberOfClassVertices;
			...
			numOfIterationAddingEdgesToGraph = 0; // initialize iteration count
			while(actualEdgesInConnS1< estEdgesConn1Simplexes) {
				
				if((actualVerticesInConnS1 < estVerticesConn1Simplexes) && (numOfIterationAddingEdgesToGraph < maximumIteration)) {
					...
					if (setOfColosInGraph.size() != 0) {
						...
						if (proposedTail != null) {
							...
							while ( (arrNewPossTriProbCount[2] < 1)  && (numOfLoopsTri < 500) && ((actualVerticesInConnS1+numberOfClassVertices) < estVerticesConn1Simplexes)) { 
							...
							if ((arrNewPossTriProbCount[2] >= 1) && ((actualVerticesInConnS1+numberOfClassVertices) < estVerticesConn1Simplexes)) {
								//Update the count of triangle since a triangle will be created
						...
						// check if it is possible to add new triangle
						while ((arrNewTriProbCount[2] < 1) && (numOfIterationRandomTri < 500) && ((actualVerticesInConnS1+numberOfClassHeadVertices+numberOfClassTailVertices) < estVerticesConn1Simplexes)) { // discontinue after trying 500 times
						...
						if ((arrNewTriProbCount[2] > 1) && ((actualVerticesInConnS1+numberOfClassHeadVertices+numberOfClassTailVertices) < estVerticesConn1Simplexes)) {
							// Update the triangle count
							...
							estVerticesConn1Simplexes = estVerticesConn1Simplexes - numberOfClassHeadVertices - numberOfClassTailVertices;
							numOfIterationAddingEdgesToGraph = 0;
			...
			numOfIterationAddingEdgesToGraph = 0; // initialize iteration count
			while ((actualEdgesInConnS1 < estEdgesConn1Simplexes) && (numOfIterationAddingEdgesToGraph < maximumIteration)) { // Case: Triangles cannot be added to the mimic graph but edges can be added to existing triangles
	...
	private void addSelfLoops(int estEdgesInput, OfferedItemByRandomProb<BitSet> distColoProposerSelfLoopInput, ObjectObjectOpenHashMap<BitSet, IntSet> mMapColourToVertexIDsInput, IColourMappingRules mColourMapperSelfLoopInput, Map<BitSet, IntSet> mMapColourToEdgeIDsInput) {
		LOGGER.info("Estimated edges: " + estEdgesInput);
		int actualEdgesSimplexes = 0;
		int iterationCountSelf = 0;
		while ((estEdgesInput > actualEdgesSimplexes) && (iterationCountSelf < maximumIteration) && (distColoProposerSelfLoopInput!=null)) {
	...
	private void updateVertexColoMap(ObjectObjectOpenHashMap<BitSet, IntSet> mMapColourToVertexIDsinput) {
		Object[] keys = mMapColourToVertexIDsinput.keys;
		for (int i=0; i < keys.length; i++) {
			if (mMapColourToVertexIDsinput.allocated[i]) {
				BitSet vertexColo = (BitSet) keys[i];
				IntSet tempSet = mMapColourToVertexIDs.get(vertexColo);
				if (tempSet == null)
					tempSet = new DefaultIntSet(Constants.DEFAULT_SIZE);
				tempSet.addAll(mMapColourToVertexIDsinput.get(vertexColo));
...
//		}
	...
	private void generateRDFTypeEdges(ObjectObjectOpenHashMap<BitSet, IntSet> mMapColourToVertexIDsForTypeEdges) {
		// Check number of remaining edges to add
		int actualEdgesMimicGraph = mMimicGraph.getEdges().size();
		int remainingEdges = mIDesiredNoOfEdges - actualEdgesMimicGraph;
		...
		Object[] keys = mMapColourToVertexIDsForTypeEdges.keys;
		for (int i = 0; i < keys.length; i++) {
			if (mMapColourToVertexIDsForTypeEdges.allocated[i]) {
				BitSet vertexColor = (BitSet) keys[i];
				Set<BitSet> classColours = mMimicGraph.getClassColour(vertexColor);
				classColorsCreateVertices.addAll(classColours);
				vertexIdsTypeEdges.addAll(mMapColourToVertexIDsForTypeEdges.get(vertexColor));
			}
		...
		/*
		 * // iterate over every vertex colour and get all class colors for(BitSet
		 * vertexColor : mMapColourToVertexIDsForTypeEdges.keySet()) { Set<BitSet>
		 * classColours = mMimicGraph.getClassColour(vertexColor);
		 * classColorsCreateVertices.addAll(classColours);
		 * vertexIdsTypeEdges.addAll(mMapColourToVertexIDsForTypeEdges.get(vertexColor))
		 * ; }
		 */
...
//		}
	 ...
	 * @return - vertex id
	 */
	private int addVertexToMimicGraph(BitSet vertexColor, ObjectObjectOpenHashMap<BitSet, IntSet> mMapColourToVertexIDsToUpdate) {
		int vertId = mMimicGraph.addVertex(vertexColor);
		IntSet setVertIDs = mMapColourToVertexIDsToUpdate.get(vertexColor); //mMapColourToVertexIDs.get(vertexColor);
		if(setVertIDs == null){
			setVertIDs = new DefaultIntSet(Constants.DEFAULT_SIZE);
