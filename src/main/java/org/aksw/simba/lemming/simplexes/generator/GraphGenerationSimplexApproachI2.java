 ...
 * Connected 2-simplexes: Probability based Triangle, Approach for adding new triangle: Edge selection probability based, and vertex is added based on probability. Adding edges to triangle: Probability based.
 * Isolated 2-simplexes: Selected based on their probability.
 * Connected 1-Simplexes: Probability based Edge, Approach for adding new edge to it: Select a head node based on probability distribution and tail node based on probability. Adding more edge : Probability based.
 ...
 * 
 */
public class GraphGenerationSimplexApproachI2 extends AbstractGraphGeneration implements IGraphGeneration {
	...
	public GraphGenerationSimplexApproachI2(int iNumberOfVertices,
	...
	public void generateGraphSingleThread(){
		...
		// get random triangle
		TriColos initialRandomTriangle = triangleDistribution.getPotentialTriangleProposer().getPotentialItem();
			...
			while(actualEdgesInTriangles< estimatedEdgesTriangle) {
				...
				if((actualVerticesInTriangles < estimatedVerticesTriangle) && (numOfIterationAddingEdgesToGraph < maximumIteration)) {
					...
					if (edgeColosSet.size() != 0) {
						...
						TriColos randomTriangle = triangleDistribution.getPotentialTriangleProposer().getPotentialItem();
			...
			while(actualEdgesInConnS1< estEdgesConnected1Simplexes) {
				...
				if((actualVerticesInConnS1 < estverticesConnected1Simplexes) && (numOfIterationAddingEdgesToGraph < maximumIteration)) {
					...
					if (setOfColosInGraph.size() != 0) {
						...
						if (proposeVertColo == null) {//Propose colors again when it is not possible to add an edge for the proposed colors
							setOfColosInGraph.remove(proposedHeadColo);
							numOfIterationAddingEdgesToGraph++;
							continue;
						}
						...
						if (proposedTail != null) {
							...
							if (arrNewPossTriProbCount == null) {
								System.out.println("Debug!");
								double[] ds = mEdgesColorsCountDistAvg.get(new EdgeColos(proposedHeadColo, proposedTail));
								System.out.println();
							}
							...
							if ((arrNewPossTriProbCount[2] >= 1) && ((actualVerticesInConnS1+numberOfClassVertices) < estverticesConnected1Simplexes)) {
								//Update the count of triangle since a triangle will be created
						...
						while ((arrNewTriProbCount[2] < 1) && (numOfIterationRandomTri < 500) && ((actualVerticesInConnS1+numberOfClassHeadVertices+numberOfClassTailVertices) < estverticesConnected1Simplexes)) { // discontinue after trying 500 times
							...
							//arrNewTriProbCount = mEdgeColosv1v2Dist.get(randomEdge.getA()).get(randomEdge.getB());// mEdgesColorsCountDistAvg.get(new EdgeColors(randomEdge.getA(), randomEdge.getB()));
						...
						if ((arrNewTriProbCount[2] > 1) && ((actualVerticesInConnS1+numberOfClassHeadVertices+numberOfClassTailVertices) < estverticesConnected1Simplexes)) {
							// Update the triangle count
