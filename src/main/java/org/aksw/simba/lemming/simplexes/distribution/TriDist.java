...
public class TriDist {
	 ...
	 */
	private OfferedItemByRandomProb<TriangleColours> potentialConnectedTriangleProposer;
	
	/**
	 * Object for storing distributions by analyzing isolated triangles found in all input graphs.
	 */
	private OfferedItemByRandomProb<TriangleColours> potentialIsolatedTriangleProposer;
	...
	public TriDist(ObjectObjectOpenHashMap<TriangleColours, double[]> mTriColoEdgesTriCountDistAvg, ObjectObjectOpenHashMap<TriangleColours, double[]> mIsolatedTriColosCountsDist, int iNoOfVersions, int mIDesiredNoOfVertices, Random mRandom){
		...
		//TODO: Logic to add values in mTriangleColorsv1v2v3
	 ...
	 * @return
	 */
	public TriangleColours proposeTriangleToAddEdge(Set<TriangleColours> setTriangleColorsMimicGraph) {
		
		//Utilize initialized object distribution for all triangles

		// get the selected triangle colors
		TriangleColours potentialTriangleColours = potentialConnectedTriangleProposer.getPotentialItem(setTriangleColorsMimicGraph);
		
		for(TriangleColours triangleColourObject: setTriangleColorsMimicGraph) {
	 ...
	 * @return
	 */
	public TriangleColours proposeIsoTriToAddEdge(Set<TriangleColours> setTriangleColorsMimicGraph) {
		
		//Utilize initialized object distribution for all triangles

		// get the selected triangle colors
		TriangleColours potentialTriangleColours = potentialIsolatedTriangleProposer.getPotentialItem(setTriangleColorsMimicGraph);
	 ...
	 */
	private OfferedItemByRandomProb<TriangleColours> initializeDistributionAllTriangles(ObjectObjectOpenHashMap<TriangleColours, double[]> mTriangleColoursTriangleCountsEdgeCountsResourceNodes) {
		
		OfferedItemByRandomProb<TriangleColours> triangleProposerOutput;
		...
		TriangleColours[] triangleColorsSampleSpace = new TriangleColours[numberOfDistinctTriangleColours];
		...
		for(int mapKeyIndex = 0; mapKeyIndex < keysTriangleColours.length ; mapKeyIndex++) {
			if(mTriangleColoursTriangleCountsEdgeCountsResourceNodes.allocated[mapKeyIndex]) {
				TriangleColours triangleColorObj = (TriangleColours) keysTriangleColours[mapKeyIndex];
		...
		ObjectDistribution<TriangleColours> potentialTriangle = new ObjectDistribution<TriangleColours>(triangleColorsSampleSpace, possEdgesInTriangle);
		if (!potentialTriangle.isEmpty()) // When samplespace is empty, initialzing the proposer with null
			triangleProposerOutput = new OfferedItemByRandomProb<TriangleColours>(potentialTriangle, mRandom);
		else
	...
	public OfferedItemByRandomProb<TriangleColours> getPotentialIsolatedTriangleProposer() {
		return potentialIsolatedTriangleProposer;
	}

	public OfferedItemByRandomProb<TriangleColours> getPotentialTriangleProposer() {
