...
import org.aksw.simba.lemming.simplexes.analysis.Connected2Simplexes;
import org.aksw.simba.lemming.simplexes.analysis.FindSelfLoops;
import org.aksw.simba.lemming.simplexes.analysis.FindTriangles;
import org.aksw.simba.lemming.simplexes.analysis.Isolated1Simplexes;
import org.aksw.simba.lemming.simplexes.analysis.Isolated2Simplexes;
import org.aksw.simba.lemming.simplexes.analysis.IsolatedSelfLoops;
import org.aksw.simba.lemming.simplexes.analysis.Simplexes0;
import org.aksw.simba.lemming.simplexes.analysis.Simplexes1ConnectedTo2Simplexes;
import org.aksw.simba.lemming.simplexes.analysis.Simplexes1Connecting2Simplexes;
import org.aksw.simba.lemming.simplexes.distribution.EdgeDist;
import org.aksw.simba.lemming.simplexes.distribution.TriDist;
import org.aksw.simba.lemming.simplexes.distribution.VertDist;
...
public class GraphGenerationSimplexApproach1 extends AbstractGraphGeneration implements IGraphGeneration {
	private static final Logger LOGGER = LoggerFactory.getLogger(GraphGenerationSimplexApproach1.class);
	...
	private int estimatedVertsConnS1; //Estimated number of edges for connected 1-simplexes
	...
	private ObjectObjectOpenHashMap<TriangleColours, double[]> mTriangleColoursTriangleEdgeCounts; // Map for storing vertex colors of triangles as keys and array of triangle and edge counts as value.
	private Set<TriangleColours> setAllTriangleColours; // Set for storing all triangles
	private ObjectObjectOpenHashMap<TriangleColours, List<IntSet>> mTriangleColorsVertexIds; // Map for tracking triangle colors and vertex IDs forming triangles using those colors 
	...
	private ObjectObjectOpenHashMap<TriangleColours, List<IntSet>> mIsolatedTriangleColorsVertexIds = new ObjectObjectOpenHashMap<TriangleColours, List<IntSet>>(); // Map for tracking triangle colors and vertex IDs forming isolated triangles using those colors
	
	//*************** Distribution objects ***************************************//
	private TriDist triangleDistribution; // store distributions of isolated and connected 2-simplexes
	private EdgeDist s1connToTriDist; // 1-simplexes connected to triangles
	private VertDist s1connToTriVertDist; // Distribution of vertices connected to triangles
	private EdgeDist s1connTriDist; // 1-simplexes connecting triangles
	private EdgeDist s1IsoDist; // isolated 1-simplexes
	private EdgeDist s1ConnDist; // connected 1-simplexes
	private VertDist s0Dist; // 0-simplexes
	
	//************* Distribution objects self loop *******************************//
	private VertDist selfLoopIsoTritDist; // Distribution of self loops for isolated triangles
	private VertDist selfLoopConnTriDist; // Distribution of self loops for connected triangles
	private VertDist selfLoops1ConnToTriDist; // Distribution of self loops for vertices connected to triangles
	private VertDist selfLoops1IsoS1; // Distribution of isolated self loops
	private VertDist selfLoopsInS1Dist; // Distribution of self loops for vertices in isolated 1-simplexes
	private VertDist selfLoopsInConnS1Dist; // Distribution of self loops for vertices in connected 1-simplexes
	...
	public GraphGenerationSimplexApproach1(int iNumberOfVertices,
			ColouredGraph[] inputGrphs, int iNumberOfThreads, long seed) {
		super();
		...
		FindTriangles computedTriangles = callMetricToGetTriangleInformation(origGrphs);
		...
		Connected2Simplexes connTriAnalysis = new Connected2Simplexes(origGrphs, mIDesiredNoOfVertices, iNoOfVersions, computedTriangles);
		...
		Isolated2Simplexes isoTriAnalysis = new Isolated2Simplexes(origGrphs, mIDesiredNoOfVertices, iNoOfVersions, computedTriangles);
		...
		triangleDistribution = new TriDist(connTriAnalysis.getmTriColoEdgesTriCountDistAvg(), isoTriAnalysis.getmIsolatedTriColoEdgesTriCountDistAvg(), iNoOfVersions, mIDesiredNoOfVertices, mRandom);
		...
		Simplexes1ConnectedTo2Simplexes s1ConnToTri = new Simplexes1ConnectedTo2Simplexes(origGrphs, mIDesiredNoOfVertices, iNoOfVersions, computedTriangles);
		...
		mTriangleColorsVertexIds = new ObjectObjectOpenHashMap<TriangleColours, List<IntSet>>();
		
		//******************* Compute Distribution of 1-simplexes connected to triangles ****************************//
		s1connToTriDist = new EdgeDist(s1ConnToTri.getmHeadColoCount(), s1ConnToTri.getmHeadColoTailColoCount() , iNoOfVersions, mRandom);
		s1connToTriVertDist = new VertDist(s1ConnToTri.getmColoCountVertConnectedToTriangle(), iNoOfVersions, mRandom);
		
		
		//***************** Simplex Analysis 1-Simplexes connecting Triangles *************************//
		Simplexes1Connecting2Simplexes s1ConnectingTri = new Simplexes1Connecting2Simplexes(origGrphs, mIDesiredNoOfVertices, iNoOfVersions, computedTriangles);
		...
		s1connTriDist = new EdgeDist(s1ConnectingTri.getmHeadColoCount(), s1ConnectingTri.getmHeadColoTailColoCount(), iNoOfVersions, mRandom);
		...
		selfLoopIsoTritDist = new VertDist(selfLoopIsoTri.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
		...
		selfLoopConnTriDist = new VertDist(selfLoopConnTri.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
		...
		selfLoops1ConnToTriDist = new VertDist(selfLoops1ConnToTri.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
		...
		selfLoops1IsoS1 = new VertDist(isoS1SelfLoopAnalysis.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
		...
		selfLoopsInS1Dist = new VertDist(selfLoopsInIsoS1.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
		
		//************ 0-Simplexes Analysis *********************************//
		Simplexes0 s0Analysis = new Simplexes0(origGrphs, mIDesiredNoOfVertices, iNoOfVersions);
		estimatedVertices0Simplexes = s0Analysis.getEstVertices();
		s0Dist = new VertDist(s0Analysis.getmColoCount0Simplex(), iNoOfVersions, mRandom);
		...
		s1ConnDist = new EdgeDist(connS1Analysis.getmHeadColoCountConnected1Simplex(), connS1Analysis.getmHeadColoTailColoCountConnected(), iNoOfVersions, mRandom);
		...
		selfLoopsInConnS1Dist = new VertDist(selfLoopsInConnS1.getmColoCountSelfLoop(), iNoOfVersions, mRandom);
	...
	private FindTriangles callMetricToGetTriangleInformation(ColouredGraph[] origGrphs) {
		// Initialize FindTriangleObject, which will collect triangles found in Input graphs.
		FindTriangles findTriObj = new FindTriangles();
	...
	private void createSetForTriangleColours() {
		...
		setAllTriangleColours = new HashSet<TriangleColours>();
	...
	public ColouredGraph generateGraph(){
		...
		// get random triangle
		TriangleColours initialRandomTriangle = getRandomTriangle();
		...
		if ((initialRandomTriangle!=null) && (estimatedEdgesTriangle >= 3) && (estimatedVerticesTriangle >=3)) { 
			...
			Set<TriangleColours> setTriangleColorsMimicGraph = new HashSet<TriangleColours>();
				...
				if(actualVerticesInTriangles < estimatedVerticesTriangle) {
					...
					if (edgesMimicGraph.size() != 0) {
						...
						if (proposedVertex3Colo != null) {
							...
							TriangleColours newPossibleTriangle = new TriangleColours( selectedVertex1Colo, selectedVertex2Colo, proposedVertex3Colo);
							...
							while ( (arrNewPossTriProbCount[3] < 1)  && (numOfLoopsTri < 100)) { // trying to create a new triangle 100 times
								...
								newPossibleTriangle = new TriangleColours( selectedVertex1Colo, selectedVertex2Colo, proposedVertex3Colo);
						...
						TriangleColours randomTriangle = getRandomTriangle();
						...
						if (arrNewTriProbCount[3] > 1) {
							...
							// Add the triangle colors to set variable
							setTriangleColorsMimicGraph.add(new TriangleColours(randomTriangle.getA(), randomTriangle.getB(), randomTriangle.getC()));
						} else {
			...
			while ((actualEdgesInTriangles< estimatedEdgesTriangle) && (numOfIterationAddingEdgesToGraph < 5000)) { // Case: Triangles cannot be added to the mimic graph but edges can be added to existing triangles
					
					if (setTriangleColorsMimicGraph.size() != 0) {
					
					// Logic for adding edges to existing triangles
					TriangleColours proposeTriangleToAddEdge = triangleDistribution.proposeTriangleToAddEdge(setTriangleColorsMimicGraph);
		...
		numOfIterationAddingEdgesToGraph = 0; // initialize iteration count
		while((estimatedEdgesIsoSelfLoop > actualEdgesSimplexes) && (potentialHeadColoProposer != null) && (numOfIterationAddingEdgesToGraph < 5000)) { // additional condition that color proposer should not be null
		...
		ObjectObjectOpenHashMap<TriangleColours, List<IntSet>> mTriangleColorsVertexIdsIsolated = new ObjectObjectOpenHashMap<TriangleColours, List<IntSet>>();
		...
		for (int i=0; i < triColoskeys.length; i++) {
			if (mTriangleColorsVertexIds.allocated[i]) {
				TriangleColours triColosToCheck = (TriangleColours) triColoskeys[i];
		...
		if (mTriangleColorsVertexIdsIsolated.size() > 0) {
			...
			while((actualEdgesSimplexes < estimatedEdges1SimplexesConnect2Simplexes) && (numOfIterationAddingEdgesToGraph < 5000)) {
				...
				if (potentialHeadColoIsolatedTri != null) { //check for tail color only if head color is not null
					...
					if (potentialTailColoIsolatedTri != null) { // try to connect triangles using 1-simplexes if tail color is also not null
						...
						TriangleColours potentialTriangleHead = null;
						
						//temporary variable to store the Triangle for proposed tail color
						TriangleColours potentialTriangleTail = null;
						...
						for (int i=0; i< triColoIsolatedkeys.length; i++) {
							if (mTriangleColorsVertexIdsIsolated.allocated[i]) {
								TriangleColours triColoIsolated = (TriangleColours) triColoIsolatedkeys[i];
		...
		while((actualEdgesSimplexes < estimatedEdges1SimplexesConnect2Simplexes) && (numOfIterationAddingEdgesToGraph < 5000)) { // check if we can add more edge
			...
			if (headColoProposercase4b != null) {
				...
				if ( (potentialHeadColocase4b!= null) && (potentialTailColocase4b!=null)) {
					...
					TriangleColours potentialTriangleHeadCase4b = null;
					TriangleColours potentialTriangleTailCase4b = null;
					...
					for (int i=0; i< connTrianglesColos.length; i++) {
						if (mTriangleColorsVertexIds.allocated[i]) {
							TriangleColours tempTriColo = (TriangleColours) connTrianglesColos[i];
		...
		OfferedItemByRandomProb<TriangleColours> potentialIsolatedTriangleProposer = triangleDistribution.getPotentialIsolatedTriangleProposer(); // get isolated triangle proposer
		...
		// Variable to track set of triangle added to the mimic graph (i.e. set of Colors of the vertices forming the triangle)
		Set<TriangleColours> setIsoTriInMimicGraph = new HashSet<TriangleColours>();
		...
		while ( ((estimatedVerticesIsolatedTriangle - actualVerticesSimplexes) >= 3) && ((estimatedEdgesIsolatedTriangle - actualEdgesSimplexes) >= 3 ) && (potentialIsolatedTriangleProposer != null) ) {
			TriangleColours possIsoTri = potentialIsolatedTriangleProposer.getPotentialItem();
			// add the selected random triangle to mimic graph			
		...
		while ((estimatedEdgesIsolatedTriangle > actualEdgesSimplexes) && (iterationCount < 5000)) {
			...
			if (setIsoTriInMimicGraph.size() > 0) {
				...
				// Logic for adding edges to existing triangles
				TriangleColours proposeTriangleToAddEdge = triangleDistribution.proposeIsoTriToAddEdge(setIsoTriInMimicGraph);
		...
		while((edgesConnected1Simplexes > actualEdgesSimplexes) && (potentialVertColoProposer != null) && (numOfIterationAddingEdgesToGraph < 5000)) {
			// check until it is possible to add more edges for 1-simplexes
		...
		while((actualEdgesSimplexes < estimatedEdgesCommon) && (potentialHeadColoCommon2Simplex != null) && (numOfIterationAddingEdgesToGraph < 5000)) {
	...
	private void addSelfLoops(int estEdgesInput, OfferedItemByRandomProb<BitSet> distColoProposerSelfLoopInput, Map<BitSet, IntSet> mMapColourToVertexIDsInput, IColourMappingRules mColourMapperSelfLoopInput, Map<BitSet, IntSet> mMapColourToEdgeIDsInput) {
		...
		while ((estEdgesInput > actualEdgesSimplexes) && (iterationCountSelf < 5000) && (distColoProposerSelfLoopInput!=null)) {
...
//		}
	 ...
	 */
	private void addTriangleToMimicGraph(TriangleColours inputTriangleColours, IColourMappingRules mColourMapperToUse, Map<BitSet, IntSet> mMapColourToVertexIDsToUpdate, Map<BitSet, IntSet> mMapColourToEdgeIDsToUpdate
			, ObjectObjectOpenHashMap<TriangleColours, List<IntSet>> mTriangleColorsVertexIdsToUpdate) {
	 ...
	 */
	private void updateMapTriangleColorsVertices(int vert1Id, int vert2Id, int vert3Id, TriangleColours inputTriangleColours, ObjectObjectOpenHashMap<TriangleColours, List<IntSet>> mTriColVertexIDsToUpdate) {
...
//	}
	 ...
	 * @param inputVertex2ID - ID for input vertex 2.
	 */
	private boolean addEdgeTriangle(BitSet inputVertex1Colo, BitSet inputVertex2Colo, int inputVertex1ID, int inputVertex2ID, Map<BitSet, IntSet> mMapColourToEdgeIDsToUpdate, IColourMappingRules mColourMapperToUse, TriangleColours inputTriangleColours) {
		boolean isEdgeFromFirstToSecondVertex = true;
		// Get edge between vertex1 and vertex 2, assuming vertex 1 is tail and vertex 2 is head
		Set<BitSet> possEdgeColov1tailv2head = mColourMapperToUse.getPossibleLinkingEdgeColours(inputVertex1Colo, inputVertex2Colo);
		if (possEdgeColov1tailv2head.size() == 0) {
	 ...
	 */
	private TriangleColours getRandomTriangle() {
		TriangleColours randomTriangleColor = null;
		
		if (setAllTriangleColours.size() != 0) {
			randomTriangleColor = setAllTriangleColours.toArray(new TriangleColours[setAllTriangleColours.size()])[mRandom.nextInt(setAllTriangleColours.size())];
		}
